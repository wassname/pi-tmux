...
Each package registers `/rename` plus a settings command named after the package:
...
language the user wrote in. Point it at a cheaper model instead with:

```
/pi-tmux naming-model openai/gpt-5-mini
/pi-tmux naming-model -          # back to the session model
```
